...
7. `compliance_check`：对文本及相关元数据执行合规知识库校验；命中高风险时暂停发布并返回整改建议。
8. `persist_result`：保存结果、节点日志、耗时和错误信息，返回统一 `SkillResult`。
...
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:4px;margin:8px 0;">
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">① validate_input<br/>入参校验</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">② create_task<br/>taskId+workflowRunId</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">③ prepare_context<br/>车型事实/品牌/素材</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">④ compose_prompt<br/>四段式组装</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">⑤ model_generate<br/>调用+退避重试</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">⑥ parse_and_validate<br/>Schema+硬约束</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(234,102,104,0.10);border:1px solid rgba(234,102,104,0.3);font-size:10.5px;text-align:center;">⑦ compliance_check<br/>三重校验·high阻断</div>
    <div style="font-size:10px;color:#8BC8EA;align-self:center;">▶</div>
    <div style="flex:1 1 84px;min-width:0;padding:6px 4px;border-radius:6px;background:rgba(0,0,0,0.04);font-size:10.5px;text-align:center;">⑧ persist_result<br/>持久化+SkillResult</div>
  </div>
  ...
  <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;">
    <div style="flex:1 1 180px;min-width:0;padding:8px 10px;border-radius:8px;background:rgba(0,0,0,0.025);border:1px solid rgba(0,0,0,0.08);font-size:11.5px;">
      <b style="color:#8BC8EA;">图片 · 领域节点</b><br/>解析描述→匹配风格→扩散生成→超分裁切<br/><b>硬约束</b>：数量=count / 尺寸=RATIO_SIZE<br/><b>合规</b>：品牌安全（乱码/竞品Logo/畸变）
    </div>
    <div style="flex:1 1 180px;min-width:0;padding:8px 10px;border-radius:8px;background:rgba(0,0,0,0.025);border:1px solid rgba(0,0,0,0.08);font-size:11.5px;">
      <b style="color:#8BC8EA;">图文 · 领域节点</b><br/>解析选题→大纲→正文→标签配图<br/><b>硬约束</b>：wordCount 误差 ≤5%<br/><b>合规</b>：title/body/tags 三重校验
    </div>
    <div style="flex:1 1 180px;min-width:0;padding:8px 10px;border-radius:8px;background:rgba(0,0,0,0.025);border:1px solid rgba(0,0,0,0.08);font-size:11.5px;">
  ...
  <div style="font-size:11px;color:#6B7280;margin-top:10px;line-height:1.6;">
    失败分支统一为四类错误码：INVALID_INPUT（①校验失败）/ MODEL_ERROR（⑤⑥失败含重试）/ RATE_LIMITED（限流）/ UNKNOWN（未归类）；⑧前任意失败必须返回 taskId。<br/>
    节点状态：pending → running → succeeded / failed；失败从最近检查点恢复，禁止重复创建任务。⑧后资产自动写入素材库（继承 taskId + 引擎类型 + journey_stage）。
  </div>
</div>
</html>
```
...
### 3.3 统一 Skill 契约（`lib/skills/types.ts`）
...
**工作流模式**：按「输入校验 → 任务创建 → 上下文准备 → Prompt 编排 → 扩散生成 → 结果校验 → 合规校验 → 持久化」节点执行，参考图、风格、比例和数量在节点间显式传递。
...
<html style="margin:0;padding:0;">
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  <div style="font-size:15px;font-weight:600;margin-bottom:2px;">AI 图片生成 · 流程图</div>
  <div style="font-size:12px;color:#6B7280;margin-bottom:12px;">/image · FR-IMG · estimatedMs 3200 · 4 阶段 · 硬约束：数量=count / 尺寸=RATIO_SIZE[ratio]</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.14);font-size:12.5px;margin-bottom:6px;">输入：prompt / style / ratio / count / referenceImage</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">validate：prompt 非空 且 1 ≤ count ≤ 8 <span style="color:#EA6668;font-weight:600;">｜失败 → INVALID_INPUT 表单提示（不发起模型调用）</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin:6px 0;">createTaskId('image') → 前端按 4 阶段渲染动效</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  ...
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">硬约束校验：数量 = count 且 尺寸 = RATIO_SIZE[ratio] <span style="color:#EA6668;font-weight:600;">｜不满足 → 重试 1 次 → 仍失败 MODEL_ERROR</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段4 超分放大并按比例裁切</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">品牌安全检测：无乱码文字 / 无虚假或竞品 Logo / 无畸变 <span style="color:#EA6668;font-weight:600;">｜不通过 → 剔除问题图，不足数量补生成</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:linear-gradient(135deg, rgba(148,216,195,0.12), rgba(148,216,195,0.22));border:1px solid rgba(148,216,195,0.3);font-size:12.5px;margin:6px 0;">返回 images[] + revisedPrompt = 「{style}风格 · {prompt}」</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin-top:6px;">网格渲染 → 用户选中 → 下载 / 收藏 / 自动入库（继承 taskId + journey_stage）</div>
</div>
</html>
```
...
**工作流模式**：按「输入校验 → 任务创建 → 选题与上下文准备 → 大纲编排 → 正文生成 → 标签与配图建议 → 结果校验 → 合规校验 → 持久化」节点执行，选题、车型事实、平台参数和正文结果逐节点传递。
...
<html style="margin:0;padding:0;">
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  <div style="font-size:15px;font-weight:600;margin-bottom:2px;">AI 图文生成 · 流程图</div>
  <div style="font-size:12px;color:#6B7280;margin-bottom:12px;">/text · FR-TXT · estimatedMs 2600 · 4 阶段 · 硬约束：wordCount 误差 ≤5% · 无 SEO 评分</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.14);font-size:12.5px;margin-bottom:6px;">输入：topic / platform / tone / length / keywords / imageSize</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">validate：topic 非空 <span style="color:#EA6668;font-weight:600;">｜失败 → INVALID_INPUT 表单提示</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin:6px 0;">createTaskId('text') → 前端按 4 阶段渲染动效</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段1 解析选题与平台（注入 journey_stage + 爆款关键词包）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段2 生成内容大纲（标题 / 导语 / 正文结构）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段3 AI 撰写正文（车型参数标注「数据来源：车型数据库」，缺失标「以官方信息为准」）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段4 生成标签与配图建议（tags + coverSuggestions）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">硬约束校验：wordCount 与正文实际中文字符数误差 ≤ 5% <span style="color:#EA6668;font-weight:600;">｜不满足 → 重试 1 次 → 仍失败 MODEL_ERROR</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(234,102,104,0.10);border:1px solid rgba(234,102,104,0.3);font-size:12.5px;margin:6px 0;">合规网关 POST /api/knowledge/validate（title / body / tags）<span style="color:#EA6668;font-weight:600;">｜命中 high → 整改建议，禁止直接发布</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:linear-gradient(135deg, rgba(148,216,195,0.12), rgba(148,216,195,0.22));border:1px solid rgba(148,216,195,0.3);font-size:12.5px;margin:6px 0;">返回 title / body / tags / coverSuggestions / wordCount</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin-top:6px;">图文混排预览（按 imageSize）→ 一键复制 / 自动入库</div>
</div>
</html>
```
...
**工作流模式**：按「输入校验 → 任务创建 → 脚本准备 → 分镜编排 → 数字人和音色合成 → 卡点与字幕合成 → 结果校验 → 合规校验 → 持久化」节点执行，脚本、分镜、音频和视频片段按依赖顺序传递。
...
<html style="margin:0;padding:0;">
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  <div style="font-size:15px;font-weight:600;margin-bottom:2px;">AI 视频生成 · 流程图</div>
  <div style="font-size:12px;color:#6B7280;margin-bottom:12px;">/video · FR-VID · estimatedMs 4200 · 4 阶段 · 硬约束：Σ storyboard[].durationSec = durationSec</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.14);font-size:12.5px;margin-bottom:6px;">输入：topic / digitalHuman / voice / videoSize / durationSec / type</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  ...
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段2 智能分镜：4 镜头（钩子→外观→座舱→CTA），单镜时长 = durationSec / 4</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">硬约束校验：Σ storyboard[].durationSec = durationSec <span style="color:#EA6668;font-weight:600;">｜不满足 → 重试 1 次 → 仍失败 MODEL_ERROR</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段3 数字人口播（合成配音与口型）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">校验：字幕与口播逐句对应 且 位于画幅安全区 <span style="color:#EA6668;font-weight:600;">｜不满足 → 标记待修复，不进入合成</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段4 卡点合成（配乐卡点 + 字幕烧录 + 输出成片）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(234,102,104,0.10);border:1px solid rgba(234,102,104,0.3);font-size:12.5px;margin:6px 0;">合规网关校验 captions（性能 / 价格 / 续航须有来源）<span style="color:#EA6668;font-weight:600;">｜命中 high → 整改建议，禁止发布</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:linear-gradient(135deg, rgba(148,216,195,0.12), rgba(148,216,195,0.22));border:1px solid rgba(148,216,195,0.3);font-size:12.5px;margin:6px 0;">返回 videoUrl / coverUrl / storyboard / captions</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin-top:6px;">主预览 + 分镜时间轴切换 + 播放控制（直播切片子流程：上传回放 → 解析高光 → N 条切片）</div>
</div>
</html>
```
...
<html style="margin:0;padding:0;">
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  <div style="font-size:15px;font-weight:600;margin-bottom:2px;">AI PPT 生成 · 流程图</div>
  <div style="font-size:12px;color:#6B7280;margin-bottom:12px;">/ppt · FR-PPT · estimatedMs 3900 · 4 阶段 · 硬约束：slides.length = pages</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.14);font-size:12.5px;margin-bottom:6px;">输入：topic / scene / template / pages</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">validate：topic 非空 且 3 ≤ pages ≤ 20 <span style="color:#EA6668;font-weight:600;">｜失败 → INVALID_INPUT</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段1 解析主题与受众（提炼汇报目标）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin:6px 0;">Prompt 组装：汇报顾问角色 + scene 受众画像 + blueprint 结构 + slides Schema</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段2 智能编排大纲（一页一个结论，按 blueprint 循环）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">硬约束校验：slides.length = pages <span style="color:#EA6668;font-weight:600;">｜不满足 → 重试 1 次 → 仍失败 MODEL_ERROR</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段3 套用视觉模板（匹配 template 风格：科技蓝/商务金/极简白/国潮红）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段4 数据可视化（按 chartType 生成图表与 notes）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">校验：图表口径 / 单位 / 时间范围 / 来源齐全 <span style="color:#EA6668;font-weight:600;">｜缺失 → 标记「待补充」，不得伪造数值</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(234,102,104,0.10);border:1px solid rgba(234,102,104,0.3);font-size:12.5px;margin:6px 0;">合规网关校验 bullets 与 notes <span style="color:#EA6668;font-weight:600;">｜命中 high → 阻断发布</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:linear-gradient(135deg, rgba(148,216,195,0.12), rgba(148,216,195,0.22));border:1px solid rgba(148,216,195,0.3);font-size:12.5px;margin:6px 0;">返回 title / template / slides（第 1 页标题固定为 topic）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin-top:6px;">主画布翻页 + 缩略图切换 + 演示模式 → 导出 PPTX / PDF（签名下载 URL）</div>
</div>
</html>
```
...
**工作流模式**：按「输入校验 → 任务创建 → 场景与人设准备 → 文案生成 → 图片与水印编排 → 预览渲染 → 结果校验 → 合规校验 → 持久化」节点执行，场景、人设、素材、水印和文案结果按依赖顺序传递。
...
<html style="margin:0;padding:0;">
<div style="background-color:transparent;box-sizing:border-box;padding:16px;font-family:'PingFang SC','Segoe UI',Arial,sans-serif;color:#1A1B1C;">
  <div style="font-size:15px;font-weight:600;margin-bottom:2px;">朋友圈图文 · 流程图</div>
  <div style="font-size:12px;color:#6B7280;margin-bottom:12px;">/moments · FR-MOM · estimatedMs 2400 · 4 阶段 · 硬约束：copy 80-180 字 / hashtags 3-5 个 / 水印一致</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.14);font-size:12.5px;margin-bottom:6px;">输入：scene / persona / imageSize / watermark</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">validate：scene 非空 <span style="color:#EA6668;font-weight:600;">｜失败 → INVALID_INPUT</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段1 解析场景与人设（结合每日内容日历推荐）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin:6px 0;">Prompt 组装：销售顾问助手角色 + offer/store_info + persona 口吻 + 输出 Schema</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段2 AI 文案撰写（共鸣开场 + 1 个核心福利点 + 咨询引导）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">硬约束校验：copy 长度 80-180 字 且 hashtags 3-5 个 <span style="color:#EA6668;font-weight:600;">｜不满足 → 重试 1 次 → 仍失败 MODEL_ERROR</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段3 智能配图匹配（按 imageSize 挑图并叠加水印）</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);font-size:12.5px;margin:6px 0;">校验：watermark 输出与用户勾选完全一致 <span style="color:#EA6668;font-weight:600;">｜不一致 → 标记异常，禁止自行追加二维码 / 电话 / Logo</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(139,200,234,0.10);border:1px solid rgba(139,200,234,0.3);font-size:12.5px;margin:6px 0;">阶段4 合规润色校验</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(234,102,104,0.10);border:1px solid rgba(234,102,104,0.3);font-size:12.5px;margin:6px 0;">合规网关：库存 / 价格 / 客户案例 / 联系方式来源核验 <span style="color:#EA6668;font-weight:600;">｜命中 high → 返回 findings，不生成可发布文案</span></div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:linear-gradient(135deg, rgba(148,216,195,0.12), rgba(148,216,195,0.22));border:1px solid rgba(148,216,195,0.3);font-size:12.5px;margin:6px 0;">返回 copy / images[] / hashtags[] / watermark[]</div>
  <div style="text-align:center;color:#8BC8EA;font-size:13px;line-height:1;">▼</div>
  <div style="padding:8px 12px;border-radius:8px;background:rgba(0,0,0,0.03);font-size:12.5px;margin-top:6px;">手机 mockup 渲染 + 点赞交互 + 一键复制（复制带「已复制」反馈）</div>
</div>
</html>
```
