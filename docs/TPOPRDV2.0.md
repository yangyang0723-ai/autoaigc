...
| 段落               | 内容                                                                   | 来源              |
| ---------------- | -------------------------------------------------------------------- | --------------- |
| ① system\_prompt | 角色设定 + 行业底线规则（广告法、平台规范、禁止编造事实）                                       | 引擎内置常量          |
| ② 行业上下文          | 车型事实 `brand_facts`、购车旅程 `journey_stage`、爆款关键词包 `viral_keywords`、门店信息 | 知识库 / 业务参数      |
| ③ user\_prompt   | `TInput` 各字段，按 `fields` 声明顺序序列化                                      | 前端表单            |
| ④ output\_schema | 目标 `TOutput` 的 JSON Schema + 硬性数值约束                                  | `types.ts` 类型定义 |
